...
wallpaperApplyScriptPath="$HOME/.config/scripts/wallpaper-apply.sh"
...
themeMenu() {
  ...
  if [ "$themeType" == "main" ]; then
    for i in "${!themeFiles[@]}"; do
      stow -t $HOME -d ${dotfilesPath}/themes -D "$(basename "${themeFiles[$i]}")"
    done
...
powerProfilesMenu() {
  ...
  profile=$(printf "%s\n" "${options[@]}" | rofi -dmenu -p "Set power profile: " $currentTheme)
